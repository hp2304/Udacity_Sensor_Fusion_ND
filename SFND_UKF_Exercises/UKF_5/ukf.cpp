...
void UKF::PredictRadarMeasurement(VectorXd* z_out, MatrixXd* S_out) {
    ...
    for(int i=0; i<(2 * n_aug + 1); ++i){
        auto px = Xsig_pred.col(i)[0];
        auto py = Xsig_pred.col(i)[1];
        auto vel = Xsig_pred.col(i)[2];
        auto yaw = Xsig_pred.col(i)[3];
    ...
    for(int i=0; i<(2*n_aug+1); ++i){
        ...
        S += ((col * col.transpose()) * weights[i]);
    }
    MatrixXd R = MatrixXd(n_z, n_z);
    R.diagonal() << pow(std_radr, 2), pow(std_radphi, 2), pow(std_radrd, 2);
    S += R;
    /**
     * Student part end
     */
